...
const NUTRIENT_ALL_KEYS = ["cal","pro","carb","fat","fib","iron","calc","zinc","b12","vitD","omega3","iod","sel","mag","pot","fol"];

const DEFAULT_GOALS = { cal:2800, pro:70, carb:400, fat:85, fib:38, iron:18, calc:1000, zinc:11, b12:2.4, vitD:15, omega3:1.6, iod:150, sel:55, mag:420, pot:3400, fol:400 };
...
function computeEntryNutrition(derivedIngredients, allFoods) {
  const t = {}; Object.keys(NUTRIENT_META).forEach(k => t[k] = 0);
  (derivedIngredients || []).forEach(ing => { const food = allFoods.find(f => f.id === ing.foodId); if (!food) return; const m = ing.amount_g / 100; Object.keys(NUTRIENT_META).forEach(k => { t[k] += (food[k] || 0) * m; }); });
  return t;
}
...
export default function NutriTrack() {
    ...
    dayLog.forEach(e => {
      ...
      if (e.type === "recipe") {
        (e.derivedIngredients || []).forEach(ing => {
          const food = allFoods.find(f => f.id === ing.foodId); if (!food) return;
          const m = ing.amount_g / 100; Object.keys(NUTRIENT_META).forEach(k => { t[k] += (food[k] || 0) * m; });
        });
        return;
      }
      const food = allFoods.find(f => f.id === e.foodId); if (!food) return;
      const m = e.amount / 100; Object.keys(NUTRIENT_META).forEach(k => { t[k] += (food[k] || 0) * m; });
    });
    return t;
  }, [dayLog, allFoods]);
  ...
  const effectiveGoals = exerciseBurn > 0 ? { ...goals,
  ...
  const addEntry = () => {
    ...
    if (editingEntryId) {
      ...
      setLogs(prev => ({ ...prev, [currentDate]: [...(prev[currentDate]||[]), { id:Date.now().toString(), foodId:selectedFood.id, foodName:selectedFood.name, amount:parseFloat(amount), meal, time:new Date().toISOString() }] }));
    }
  ...
  const logRecipe = () => {
    ...
    const di = selectedRecipe.ingredients.map(ing => ({ foodId:ing.foodId, foodName:ing.foodName, amount_g:Math.round(ing.amount_g*frac*10)/10 }));
  ...
  const logSuppStack = () => {
    ...
    setLogs(prev => ({ ...prev, [currentDate]: [...(prev[currentDate]||[]), { id:Date.now().toString(), type:"supplement", stackId:suppLogStack.id, stackName:suppLogStack.name, items, time:new Date().toISOString(), meal:"" }] }));
  ...
  const logOneOff = () => {
    ...
    setLogs(prev => ({ ...prev, [currentDate]: [...(prev[currentDate]||[]), { id:Date.now().toString(), type:"supplement", stackId:null, stackName:"One-off", items:[item], time:new Date().toISOString(), meal:"" }] }));
  ...
  const recipeSubtotal = (e, field) => (e.derivedIngredients||[]).reduce((s,ing) => { const f=allFoods.find(x=>x.id===ing.foodId); return s+(f?(f[field]||0)*ing.amount_g/100:0); }, 0);
  // ── LOG VIEW ──────────────────────────────────────────────────────────
  if (view === "log") {
      ...
      <div style={S.app}>
        ...
        <div style={{...S.section,paddingBottom:240}}>
          {MEALS.map(m => {
            ...
            const mealCals = entries.reduce((sum,e) => { if(e.type==="recipe") return sum+computeEntryNutrition(e.derivedIngredients||[],allFoods).cal; const f=allFoods.find(x=>x.id===e.foodId); return sum+(f?f.cal*e.amount/100:0); }, 0);
            return (
              <div key={m}>
                ...
                {entries.map(e => {
                  if (e.type === "recipe") { const nut=computeEntryNutrition(e.derivedIngredients||[],allFoods); return (
                    <SwipeableEntry key={e.id} onDelete={() => removeEntry(e.id)}>
                      <div style={S.entry}><div style={{flex:1}}><div style={{...S.entryName,color:"#a78bfa"}}>📖 {e.recipeName}</div><div style={S.entryDet}>{e.servings} {e.servings===1?"serving":"servings"}</div></div>
                      <div style={{display:"flex",alignItems:"center"}}><span style={S.entryCal}>{Math.round(nut.cal)} kcal</span><button style={S.delBtn} onClick={() => removeEntry(e.id)}>×</button></div></div>
                    </SwipeableEntry>); }
                  ...
                  return (
                    <SwipeableEntry key={e.id} onDelete={() => removeEntry(e.id)}>
                      ...
                      <div style={{display:"flex",alignItems:"center"}}><span style={S.entryCal}>{f?Math.round(f.cal*e.amount/100):"-"} kcal</span><button style={S.delBtn} onClick={() => removeEntry(e.id)}>×</button></div></div>
                    </SwipeableEntry>);
                })}
              </div>
            );
          ...
          {suppEntries.length > 0 && (
            <div>
              ...
              {suppEntries.map(e => (
                <SwipeableEntry key={e.id} onDelete={() => removeEntry(e.id)}>
                  <div style={{...S.entry,background:"#0f0a1e"}}>
                    <div style={{flex:1}}><div style={{fontSize:14,fontWeight:500,color:"#c4b5fd"}}>💊 {e.stackName}</div><div style={{fontSize:12,color:"#64748b"}}>{suppItemSummary(e.items)}</div></div>
                    <div style={{display:"flex",alignItems:"center"}}><span style={{fontSize:12,color:"#64748b",marginRight:4}}>{new Date(e.time).toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}</span><button style={S.delBtn} onClick={() => removeEntry(e.id)}>×</button></div>
                  </div>
                </SwipeableEntry>
              ))}
            </div>
          ...
          {dayLog.filter(e=>e.type==="exercise").length > 0 && (
            <div>
              ...
              {dayLog.filter(e=>e.type==="exercise").map(e => (
                <SwipeableEntry key={e.id} onDelete={() => removeEntry(e.id)}>
                  <div style={{...S.entry,background:"#0a0f1a"}}><div style={{flex:1}}><div style={{fontSize:14,fontWeight:500,color:"#4ade80"}}>{e.activity}</div><div style={{fontSize:12,color:"#64748b"}}>{e.duration_min} min</div></div>
                  <div style={{display:"flex",alignItems:"center"}}><span style={{fontSize:13,fontWeight:600,color:"#4ade80"}}>−{e.calories_burned} kcal</span><button style={S.delBtn} onClick={() => removeEntry(e.id)}>×</button></div></div>
                </SwipeableEntry>
              ))}
            </div>
          )}
    ...
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          ...
          {[...MACROS,...MICROS].map(k=>{const meta=NUTRIENT_META[k];return(<div key={k} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 0",borderBottom:"1px solid #1e293b"}}><div><div style={{fontSize:14,fontWeight:500,color:"#e2e8f0"}}>{meta.label}</div><div style={{fontSize:11,color:"#64748b"}}>{meta.unit}</div></div><input style={{...S.input,width:90,textAlign:"right",padding:"8px 12px"}} type="number" inputMode="decimal" value={goals[k]} onChange={e=>setGoals(prev=>({...prev,[k]:parseFloat(e.target.value)||0}))}/></div>);})}
  ...
  const handleExportData = () => {
    ...
    for (const dk of allDayKeys) {
      ...
      for (const e of dayEntries) {
        ...
        if (e.type === "recipe") {
          (e.derivedIngredients || []).forEach(ing => {
            const f = allFoods.find(x => x.id === ing.foodId); if (!f) return;
            const m = ing.amount_g / 100;
            NUTRIENT_ALL_KEYS.forEach(k => { food[k] += (f[k] || 0) * m; });
          });
        } else {
          const f = allFoods.find(x => x.id === e.foodId); if (!f) continue;
    ...
    triggerDownload(csvBody,                     `nutritrack-daily-${exportDateStr}.csv`,  "text/csv;charset=utf-8;");
    triggerDownload(JSON.stringify(jsonObj, null, 2), `nutritrack-full-${exportDateStr}.json`, "application/json;charset=utf-8;");
    ...
    setExportConfirm({
      csvFile:    `nutritrack-daily-${exportDateStr}.csv`,
      jsonFile:   `nutritrack-full-${exportDateStr}.json`,
    ...
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          ...
          {(profile.name||profile.weightKg)&&(<div style={{...S.card,background:"#0f172a"}}><div style={{fontSize:13,fontWeight:700,color:"#94a3b8",marginBottom:10,textTransform:"uppercase",letterSpacing:"0.05em"}}>Summary</div>{profile.name&&<div style={{fontSize:14,color:"#e2e8f0",marginBottom:4}}>👤 {profile.name}</div>}{profile.weightKg&&<div style={{fontSize:14,color:"#e2e8f0",marginBottom:4}}>⚖️ {profile.weightKg} kg</div>}{profile.age&&<div style={{fontSize:14,color:"#e2e8f0",marginBottom:4}}>🎂 {profile.age} years old</div>}{profile.sex&&<div style={{fontSize:14,color:"#e2e8f0",marginBottom:4}}>⚧ {profile.sex}</div>}{!profile.weightKg&&<div style={{fontSize:12,color:"#f59e0b",marginTop:6}}>⚠️ Add your weight to enable accurate exercise calorie estimates</div>}</div>)}
          ...
          <div style={S.card}>
            ...
            {exportConfirm && (
              <div style={{marginTop:14,background:"#0a1a14",border:"1px solid #065f46",borderRadius:10,padding:"12px 14px"}}>
                ...
                <div style={{fontSize:12,color:"#6ee7b7",marginBottom:4}}>📄 {exportConfirm.csvFile}</div>
                <div style={{fontSize:12,color:"#6ee7b7",marginBottom:10}}>📋 {exportConfirm.jsonFile}</div>
  ...
  if (view === "proDetail") {
    ...
    dayLog.forEach(e=>{if(e.type==="exercise"||e.type==="supplement")return;if(e.type==="recipe"){(e.derivedIngredients||[]).forEach(ing=>{const f=allFoods.find(x=>x.id===ing.foodId);if(!f)return;AA_KEYS.forEach(k=>{aaTotals[k]+=(f[k]||0)*ing.amount_g/100;});});return;}const f=allFoods.find(x=>x.id===e.foodId);if(!f)return;AA_KEYS.forEach(k=>{aaTotals[k]+=(f[k]||0)*e.amount/100;});});
    let limitingKey="aaLys",lowestPct=Infinity; AA_KEYS.forEach(k=>{const p=(aaTotals[k]/AA_EAR[k])*100;if(p<lowestPct){lowestPct=p;limitingKey=k;}});
    const proCont=dayLog.flatMap(e=>{if(e.type==="exercise")return[];if(e.type==="supplement"){const v=suppContrib(e,"pro");return v>0?[{name:`💊 ${e.stackName}`,label:"supplement",value:v,isSupp:true}]:[]; }if(e.type==="recipe"){const n=computeEntryNutrition(e.derivedIngredients||[],allFoods);return n.pro?[{name:`📖 ${e.recipeName}`,label:`${e.servings} srv`,value:n.pro}]:[];}const f=allFoods.find(x=>x.id===e.foodId);return f&&f.pro?[{name:e.foodName,label:`${e.amount}g`,value:f.pro*e.amount/100}]:[];}).sort((a,b)=>b.value-a.value);
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          <div style={{...S.card,textAlign:"center"}}><Ring value={totals.pro} max={goals.pro} size={100} stroke={8} color={NUTRIENT_META.pro.color}><text x="50%" y="45%" textAnchor="middle" fill="#e2e8f0" fontSize={18} fontWeight={700}>{Math.round(totals.pro*10)/10}</text><text x="50%" y="62%" textAnchor="middle" fill="#64748b" fontSize={10}>/ {goals.pro}g</text></Ring><div style={{marginTop:12,fontSize:14,color:pct("pro")>=100?"#10b981":pct("pro")>=60?"#f59e0b":"#ef4444"}}>{pct("pro")}% of daily goal</div></div>
          <div style={S.card}><div style={{fontSize:13,fontWeight:700,color:"#94a3b8",marginBottom:4,textTransform:"uppercase",letterSpacing:"0.05em"}}>Essential Amino Acids</div><div style={{fontSize:11,color:"#475569",marginBottom:14}}>vs IOM EAR for 70kg adult</div>
            {AA_KEYS.map(k=>{const val=aaTotals[k],ear=AA_EAR[k],bp=Math.min((val/ear)*100,100),rp=Math.round((val/ear)*100),isLimit=k===limitingKey,col=rp>=100?"#10b981":rp>=60?"#3B82F6":rp>=30?"#f59e0b":"#ef4444";return(<div key={k} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:13,color:"#e2e8f0",fontWeight:isLimit?700:400}}>{AA_LABELS[k]}</span>{isLimit&&<span style={{fontSize:9,fontWeight:700,color:"#f59e0b",background:"rgba(245,158,11,0.15)",borderRadius:4,padding:"1px 5px"}}>LIMITING</span>}</div><div style={{textAlign:"right"}}><span style={{fontSize:12,fontWeight:600,color:col}}>{rp}%</span><span style={{fontSize:10,color:"#475569",marginLeft:5}}>{Math.round(val*100)/100}g / {ear}g</span></div></div><div style={{height:6,borderRadius:3,background:"#1e293b",overflow:"hidden"}}><div style={{height:"100%",width:`${bp}%`,background:col,borderRadius:3,transition:"width 0.5s ease"}}/></div></div>);})}
          </div>
  ...
  if (view === "calDetail") {
    ...
    const calCont=dayLog.flatMap(e=>{if(e.type==="exercise")return[];if(e.type==="supplement"){const v=suppContrib(e,"cal");return v>0?[{name:`💊 ${e.stackName}`,label:"supplement",value:v,isSupp:true}]:[]; }if(e.type==="recipe"){const n=computeEntryNutrition(e.derivedIngredients||[],allFoods);return n.cal?[{name:`📖 ${e.recipeName}`,label:`${e.servings} srv`,value:n.cal}]:[];}const f=allFoods.find(x=>x.id===e.foodId);return f?[{name:e.foodName,label:`${e.amount}g`,value:f.cal*e.amount/100}]:[];}).sort((a,b)=>b.value-a.value);
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          <div style={{...S.card,textAlign:"center"}}><Ring value={totals.cal} max={goals.cal} size={100} stroke={8} color={NUTRIENT_META.cal.color}><text x="50%" y="45%" textAnchor="middle" fill="#e2e8f0" fontSize={18} fontWeight={700}>{Math.round(totals.cal)}</text><text x="50%" y="62%" textAnchor="middle" fill="#64748b" fontSize={10}>/ {goals.cal} kcal</text></Ring><div style={{marginTop:12,fontSize:14,color:pct("cal")>=100?"#10b981":pct("cal")>=60?"#f59e0b":"#ef4444"}}>{pct("cal")}% of daily goal</div></div>
  ...
  if (view === "fibDetail") {
    const fibSol=dayLog.reduce((s,e)=>{if(e.type==="exercise"||e.type==="supplement")return s;if(e.type==="recipe")return s+recipeSubtotal(e,"fibSol");const f=allFoods.find(x=>x.id===e.foodId);return s+(f?(f.fibSol||0)*e.amount/100:0);},0);
    const fibInsol=dayLog.reduce((s,e)=>{if(e.type==="exercise"||e.type==="supplement")return s;if(e.type==="recipe")return s+recipeSubtotal(e,"fibInsol");const f=allFoods.find(x=>x.id===e.foodId);return s+(f?(f.fibInsol||0)*e.amount/100:0);},0);
    const fibT=fibSol+fibInsol;
    const fibCont=dayLog.flatMap(e=>{if(e.type==="exercise"||e.type==="supplement")return[];if(e.type==="recipe"){const n=computeEntryNutrition(e.derivedIngredients||[],allFoods);if(!n.fib)return[];return[{name:`📖 ${e.recipeName}`,label:`${e.servings} srv`,total:n.fib,sol:recipeSubtotal(e,"fibSol"),insol:recipeSubtotal(e,"fibInsol")}];}const f=allFoods.find(x=>x.id===e.foodId);if(!f||!f.fib)return[];return[{name:e.foodName,label:`${e.amount}g`,total:f.fib*e.amount/100,sol:(f.fibSol||0)*e.amount/100,insol:(f.fibInsol||0)*e.amount/100}];}).sort((a,b)=>b.total-a.total);
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          <div style={{...S.card,textAlign:"center"}}><Ring value={totals.fib} max={goals.fib} size={100} stroke={8} color={NUTRIENT_META.fib.color}><text x="50%" y="45%" textAnchor="middle" fill="#e2e8f0" fontSize={18} fontWeight={700}>{Math.round(totals.fib*10)/10}</text><text x="50%" y="62%" textAnchor="middle" fill="#64748b" fontSize={10}>/ {goals.fib}g</text></Ring><div style={{marginTop:12,fontSize:14,color:pct("fib")>=100?"#10b981":pct("fib")>=60?"#f59e0b":"#ef4444"}}>{pct("fib")}% of daily goal</div></div>
  ...
  if (view === "fatDetail") {
    const fatSat=dayLog.reduce((s,e)=>{if(e.type==="exercise"||e.type==="supplement")return s;if(e.type==="recipe")return s+recipeSubtotal(e,"fatSat");const f=allFoods.find(x=>x.id===e.foodId);return s+(f?(f.fatSat||0)*e.amount/100:0);},0);
    const fatMufa=dayLog.reduce((s,e)=>{if(e.type==="exercise"||e.type==="supplement")return s;if(e.type==="recipe")return s+recipeSubtotal(e,"fatMufa");const f=allFoods.find(x=>x.id===e.foodId);return s+(f?(f.fatMufa||0)*e.amount/100:0);},0);
    const fatPufa=dayLog.reduce((s,e)=>{if(e.type==="exercise"||e.type==="supplement")return s;if(e.type==="recipe")return s+recipeSubtotal(e,"fatPufa");const f=allFoods.find(x=>x.id===e.foodId);return s+(f?(f.fatPufa||0)*e.amount/100:0);},0);
    const fatT=fatSat+fatMufa+fatPufa;
    const fatCont=dayLog.flatMap(e=>{if(e.type==="exercise"||e.type==="supplement")return[];if(e.type==="recipe"){const n=computeEntryNutrition(e.derivedIngredients||[],allFoods);if(!n.fat)return[];return[{name:`📖 ${e.recipeName}`,label:`${e.servings} srv`,total:n.fat,sat:recipeSubtotal(e,"fatSat"),mufa:recipeSubtotal(e,"fatMufa"),pufa:recipeSubtotal(e,"fatPufa")}];}const f=allFoods.find(x=>x.id===e.foodId);if(!f||!f.fat)return[];return[{name:e.foodName,label:`${e.amount}g`,total:f.fat*e.amount/100,sat:(f.fatSat||0)*e.amount/100,mufa:(f.fatMufa||0)*e.amount/100,pufa:(f.fatPufa||0)*e.amount/100}];}).sort((a,b)=>b.total-a.total);
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          <div style={{...S.card,textAlign:"center"}}><Ring value={totals.fat} max={goals.fat} size={100} stroke={8} color={NUTRIENT_META.fat.color}><text x="50%" y="45%" textAnchor="middle" fill="#e2e8f0" fontSize={18} fontWeight={700}>{Math.round(totals.fat*10)/10}</text><text x="50%" y="62%" textAnchor="middle" fill="#64748b" fontSize={10}>/ {goals.fat}g</text></Ring><div style={{marginTop:12,fontSize:14,color:pct("fat")>=100?"#10b981":pct("fat")>=60?"#f59e0b":"#ef4444"}}>{pct("fat")}% of daily goal</div></div>
  ...
  if (view === "detail" && detailNutrient) {
    ...
    const cont=dayLog.flatMap(e=>{if(e.type==="exercise")return[];if(e.type==="supplement"){const v=suppContrib(e,k);return v>0?[{name:`💊 ${e.stackName}`,label:"supplement",value:v,isSupp:true}]:[]; }if(e.type==="recipe"){const n=computeEntryNutrition(e.derivedIngredients||[],allFoods);return n[k]?[{name:`📖 ${e.recipeName}`,label:`${e.servings} srv`,value:n[k]}]:[];}const f=allFoods.find(x=>x.id===e.foodId);return f?[{name:e.foodName,label:`${e.amount}g`,value:(f[k]||0)*e.amount/100}]:[];}).sort((a,b)=>b.value-a.value);
    return (
      <div style={S.app}>
