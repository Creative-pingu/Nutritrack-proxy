...
export default function NutriTrack() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const parseErrors = [];
      const resolve = (val, fallback, name) => {
        if (val === PARSE_ERROR) { parseErrors.push(`${name}: invalid JSON (corrupted value)`); return fallback; }
        return val;
      };
      const safeL  = resolve(l,  {},                       "logs");
      const safeC  = resolve(c,  [],                       "customFoods");
      const safeP  = resolve(p,  DEFAULT_PROFILE,          "profile");
      const safeEr = resolve(er, DEFAULT_EX_RATIO,         "exRatio");
      const safeRc = resolve(rc, [],                       "recipes");
      const safeSs = resolve(ss, DEFAULT_SUPPLEMENT_STACKS,"supplementStacks");
  ...
  // Phase 6b — refresh storage estimate after any save
  useEffect(() => {
    if (!loaded) return;
    ...
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          ...
          {/* Storage Health (Phase 6b) */}
          {(()=>{
            const bytes  = storageEstimate?.usageBytes ?? null;
            const quota  = storageEstimate?.quotaBytes ?? null;
            const usageStr = bytes == null ? null
              : bytes < 1024         ? `${bytes} bytes`
              : bytes < 1024 * 1024  ? `${(bytes / 1024).toFixed(1)} KB`
              :                        `${(bytes / 1024 / 1024).toFixed(2)} MB`;
            const quotaStr = quota == null ? null
              : quota < 1024 * 1024  ? `${Math.round(quota / 1024)} KB`
              :                        `${(quota / 1024 / 1024).toFixed(1)} MB`;
            const pct     = (bytes != null && quota != null && quota > 0) ? (bytes / quota) * 100 : null;
            ...
            const statusText  = { green:"Storage healthy", yellow:"Storage approaching limit — consider exporting and reviewing data", red:"Storage near full — export now", unknown:"Storage usage information not available on this device" }[status];
            return (
              <>
                ...
                <div style={S.card}>
                  {storageEstimate ? (
                    <>
                      <div style={{fontSize:13,color:"#94a3b8",marginBottom:8}}>Using <span style={{color:"#e2e8f0",fontWeight:600}}>{usageStr}</span> of approximately <span style={{color:"#e2e8f0",fontWeight:600}}>{quotaStr}</span> available</div>
          ...
          <details>
            ...
            <div style={S.card}>
              <div style={{fontSize:12,color:"#64748b",marginBottom:12,lineHeight:1.5}}>One-use-per-session failure injection for testing the storage validation banner. Reload after triggering.</div>
              <button
                ...
                onClick={()=>{
                  ...
                  console.log("[NutriTrack][debug] Corrupted value injected into nt-logs. Reload to see validation banner.");
                }}>
              ...
              <button
                ...
                onClick={()=>{
                  ...
                  localStorage.setItem(STORAGE_KEYS.logs, JSON.stringify([])); // logs as array = shape mismatch
                  setDbgShapeUsed(true);
                  console.log("[NutriTrack][debug] Shape mismatch injected into nt-logs (array instead of object). Reload to see validation banner.");
                }}>
