...
function normaliseUnit(token) {
  ...
  return t;  // already canonical: g, kg, ml, l, tsp, tbsp, cup, pinch
}
...
function parseIngredientLine(rawLine) {
  ...
  // 2. Strip trailing parenthetical: "flour (sifted)" -> "flour"
...
/* ──────────────────────────────────────────────────────────────────────────
 * Phase 5c — regex parser test cases
 * ──────────────────────────────────────────────────────────────────────────
 * The following inputs were verified against parseIngredientLine during
 * development. Each line shows: input -> expected output.
 * 27/27 passed when last validated. To re-run, extract parseIngredientLine
 ...
 *   "flour, sifted"           -> null  (annotation, no quantity → null)
 *   "100g flour, sifted"      -> { name:"flour",       amount:100, unit:"g"     }
 *   "dash of vanilla"         -> { name:"vanilla",     amount:1,   unit:"pinch" }
 *
 * Real Notion data (Harissa Roasted Chickpeas) — informational, not asserted:
 *   "200g cherry tomatoes"        -> { name:"cherry tomatoes", amount:200, unit:"g" }
 *   "1 capsicum"                  -> { name:"capsicum",  amount:120, unit:"g" }
 *   "1 can chopped tomatoes"      -> { name:"can chopped tomato", amount:100, unit:"g" }
 *                                    (review screen will need manual match)
 *   "700g chickpeas"              -> { name:"chickpeas", amount:700, unit:"g" }
 *   "2 tbsp rose harissa paste"   -> { name:"rose harissa paste", amount:2, unit:"tbsp" }
 *   "Hummus"                      -> null  (no quantity, garnish line)
 *   "Sourdough"                   -> null
 *   "Parsley"                     -> null
 *   "Pine nuts"                   -> null
 */
