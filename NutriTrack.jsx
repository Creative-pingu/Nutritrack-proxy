...
export default function NutriTrack() {
    ...
    dayLog.forEach(e => {
      ...
      if (e.type === "recipe") {
        (e.derivedIngredients || []).forEach(ing => {
          const food = allFoodsForRender.find(f => f.id === ing.foodId); if (!food) return;
          const m = ing.amount_g / 100; Object.keys(NUTRIENT_META).forEach(k => { t[k] += (food[k] || 0) * m; });
        });
        return;
      }
      const food = allFoodsForRender.find(f => f.id === e.foodId); if (!food) return;
  ...
  const saveEditedRecipeEntry = () => {
    ...
    if (recipe) {
      ...
      di = recipe.ingredients.map(ing => ({ foodId: ing.foodId, foodName: ing.foodName, amount_g: Math.round(ing.amount_g * frac * 10) / 10 }));
    }
  ...
  const saveEditedExerciseEntry = () => {
    ...
    const burn = editLogBurn !== "" ? (parseInt(editLogBurn) || 0) : editingLogEntry.calories_burned;
    setLogs(prev => ({ ...prev, [currentDate]: (prev[currentDate] || []).map(e =>
  ...
  const saveEditedSuppEntry = () => {
    ...
    const items = editLogSuppItems.map(i => {
      ...
      return { name: i.name, dose_amount: (!isNaN(d) && i.doseOverride.trim() !== "") ? d : i.dose_amount, dose_unit: i.dose_unit, nutrients: { ...i.nutrients } };
    });
    setLogs(prev => ({ ...prev, [currentDate]: (prev[currentDate] || []).map(e =>
  ...
  const recipeSubtotal = (e, field) => (e.derivedIngredients||[]).reduce((s,ing) => { const f=allFoodsForRender.find(x=>x.id===ing.foodId); return s+(f?(f[field]||0)*ing.amount_g/100:0); }, 0);
  // ── LOG VIEW ──────────────────────────────────────────────────────────
  if (view === "log") {
      ...
      <div style={S.app}>
        ...
        <div style={{...S.section,paddingBottom:240}}>
          {MEALS.map(m => {
            ...
            const mealCals = entries.reduce((sum,e) => { if(e.type==="recipe") return sum+computeEntryNutrition(e.derivedIngredients||[],allFoodsForRender).cal; const f=allFoodsForRender.find(x=>x.id===e.foodId); return sum+(f?f.cal*e.amount/100:0); }, 0);
            return (
              <div key={m}>
  ...
  if (view === "editLoggedExercise" && editingLogEntry) {
    const dur = parseFloat(editLogDuration) || 0;
    const displayBurn = editLogBurn !== "" ? (parseInt(editLogBurn) || 0) : editingLogEntry.calories_burned;
    return (
      <div style={S.app}>
        <div style={S.header}>
          <button style={{background:"none",border:"none",color:"#94a3b8",fontSize:15,cursor:"pointer"}} onClick={() => { setEditingLogEntry(null); setView("log"); }}>← Back</button>
        ...
        <div style={S.section}>
          ...
          <div style={S.card}>
            ...
            <div style={{background:"#0a0f1a",borderRadius:10,padding:14,marginBottom:16}}>
              ...
              <div style={{fontSize:28,fontWeight:700,color:"#4ade80",marginBottom:12}}>{displayBurn} kcal</div>
