...
export default function NutriTrack() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Phase 6b — shape validation
      const vFails = validateStorageShapes({ logs: l, recipes: rc, customFoods: c, profile: p, exRatio: er, supplementStacks: ss });
      if (vFails.length > 0) {
        vFails.forEach(f => console.warn("[NutriTrack] Storage validation failure:", f));
        setValidationWarning(true);
      }
      // Record successful validation timestamp (even if shape issues found — we still loaded)
    ...
    return (
      <div style={S.app}>
        ...
        <div style={S.section}>
          ...
          {/* Storage Health (Phase 6b) */}
          {(()=>{
            const usageKB = storageEstimate ? Math.round(storageEstimate.usageBytes / 1024) : null;
            const quotaMB = storageEstimate ? Math.round(storageEstimate.quotaBytes / 1024 / 1024 * 10) / 10 : null;
            const pct     = storageEstimate ? (storageEstimate.usageBytes / storageEstimate.quotaBytes) * 100 : null;
            ...
            return (
              <>
                ...
                <div style={S.card}>
                  {storageEstimate ? (
                    <>
                      <div style={{fontSize:13,color:"#94a3b8",marginBottom:8}}>Using <span style={{color:"#e2e8f0",fontWeight:600}}>{usageKB} KB</span> of approximately <span style={{color:"#e2e8f0",fontWeight:600}}>{quotaMB} MB</span> available</div>
