...
// recipe database ID in its env vars; PWA only needs the URL.
//
// Security model: Origin allowlist on the Worker side. No shared secret in
// client code, by design — the JSX is hosted on a public GitHub Pages repo,
// where any embedded secret would be readable. The Origin gate stops casual
// abuse from real browsers on other sites; it does NOT stop a determined
// attacker using a non-browser HTTP client (curl, scripts) since Origin
// can be spoofed there. Recipe data is not sensitive and the Notion
// integration is read-only, so this is an accepted threat model.
//
// Reversibility: if recipe data later becomes sensitive, move Pages to a
// private repo (GitHub Pro), re-add WORKER_AUTH_SECRET here, and re-enable
// the auth check on the Worker side. ~30 minutes of work.
...
const UNIT_TO_G = {
  ...
  cup:240, cups:240,
  // Imperial — useful for English-language recipes (US/UK)
...
// ── INGREDIENT PARSER CONFIG ──────────────────────────────────────────────
// Phase 5c: regex-based ingredient parser replaces the Claude API call. The
// old parser is kept around (unused unless this toggle flips) so we can do
// side-by-side comparisons during 5b validation against real Notion data.
// Will be removed entirely as part of Phase 6 cleanup once the regex parser
// is proven good enough.
const USE_REGEX_PARSER = true;

// Typical-weight lookup for unit-less countable items. Used by the regex
// parser to convert "2 medium onions" into a gram quantity. The food matcher
// then matches the ingredient name against the food DB; if no match, the
// review screen flags it for manual resolution.
//
// Only include items that genuinely appear in vegan recipes Nick imports.
// Don't try to be comprehensive — items not in the table fall through to a
// 100g default with a flag, which the review screen handles.
//
// Sizes default to "medium" when not specified. Where size isn't meaningful
// (garlic clove, lemon), only one entry is provided.
const TYPICAL_WEIGHT_G = {
  ...
  "onion large":        150,
  "garlic clove":         3,
  "garlic":               3,  // when written as "3 garlic" with no "clove"
  "shallot":             30,
  "spring onion":        15,
  "leek":               150,
  "leek stick":         150,  // "1 stick leek"
  // tomatoes
  "tomato":             120,  // medium
  "tomato small":        70,
  "tomato medium":      120,
  ...
  "capsicum":           120,
  "cucumber":           200,
  // herbs (small, easy to over-count without these)
  "bay leaf":             0.5,
  "thyme sprig":          1,
  "thyme":                1,
  "rosemary sprig":       2,
...
// Unit-like count nouns to strip when parsing "3 garlic cloves" → "garlic" (×3)
// These are NOT real units — they're count words that appear after the number
// but aren't measurements. The parser collapses "3 garlic cloves" into a count
// of 3 garlic items. The lookup in TYPICAL_WEIGHT_G uses the joined form
// (e.g. "garlic clove" → 3g) when present, otherwise the bare noun.
...
// Size adjectives that prefix a countable item, e.g. "2 medium onions"
const SIZE_ADJECTIVES = new Set(["small", "medium", "large"]);

// Leading qualifiers to strip off the front of an ingredient line
const LEADING_QUALIFIERS = /^(about|approximately|approx\.?|roughly|around|~)\s+/i;

// Preparation participles/verbs that follow a food name and should be stripped.
// "3 cloves garlic minced" → name should be "garlic", not "garlic minced".
// These are stripped from the END of the name token list (after count noun
// removal), repeatedly, so combinations like "finely chopped" are handled.
const PREP_MODIFIERS = new Set([
  "minced", "chopped", "diced", "sliced", "crushed", "grated", "shredded",
  "peeled", "cubed", "halved", "quartered", "julienned", "mashed", "ground",
  "fresh", "dried", "frozen", "raw", "cooked", "roasted", "toasted",
  "finely", "coarsely", "thinly", "thickly", "lightly", "freshly",
  "optional", "divided", "softened", "melted", "chilled", "cold", "warm", "hot",
]);

// ── FUZZY FOOD MATCHING ───────────────────────────────────────────────────
// Words that, when appearing as a SUFFIX in the ingredient name but NOT in
// the candidate food name, indicate a different food. e.g. food=
// "Red Wine", ingredient="red wine vinegar" → reject (ingredient is vinegar,
// not wine). But food="Olive Oil", ingredient="olive oil" → accept (the
// modifier is part of both names).
const FOOD_IDENTITY_MODIFIERS = new Set([
  "vinegar", "juice", "oil", "paste", "sauce", "powder", "extract",
  "syrup", "butter", "cream", "milk", "flour", "stock", "broth", "wine",
]);

function isModifierMismatch(ingredientName, foodBaseName) {
  // Tokenise both. If the ingredient ends with a modifier word that the
  // food name doesn't contain, the match is wrong.
  ...
  if (ingTokens.length === 0) return false;
  const last = ingTokens[ingTokens.length - 1];
  if (!FOOD_IDENTITY_MODIFIERS.has(last)) return false;
  // Modifier word in ingredient. Mismatch if food doesn't contain it.
...
function fuzzyMatchFood(name, allFoods) {
  ...
  // Reject if the ingredient name has a trailing food-identity modifier
  // (vinegar, juice, etc.) that the food name lacks.
  m = allFoods.find(f => {
    const base = f.name.toLowerCase().split(" (")[0];
    if (base.length <= 3) return false;
    if (!n.includes(base)) return false;
    if (isModifierMismatch(n, base)) return false;
    return true;
  });
  if (m) return m;
  ...
  m = allFoods.find(f => {
    ...
    if (words.length === 0 || !words.every(w => n.includes(w))) return false;
    const base = f.name.toLowerCase().split(" (")[0];
    if (isModifierMismatch(n, base)) return false;
    return true;
  });
...
// ── WORKER API FETCHERS ───────────────────────────────────────────────────
// All requests go through the Cloudflare Worker. The Worker enforces an
// Origin allowlist and proxies read-only calls to Notion. See worker.js for
...
// ── REGEX-BASED INGREDIENT PARSER (Phase 5c) ──────────────────────────────
// Drop-in replacement for parseIngredientsWithClaude. Runs entirely client-
// side. Produces the same { name, amount, unit } output shape so the existing
// review screen and food matcher work unchanged.
//
// Behaviour summary:
//   - Strips leading qualifiers ("about 50g", "~2 tbsp")
//   - Strips trailing parentheticals ("flour (sifted)" -> "flour")
//   - Handles ranges by taking the lower number ("2-3 onions" -> 2)
//   - Handles ASCII fractions (1/2), unicode fractions (½), decimals (1.5),
//     and decimal-comma (1,5)
//   - For unit-less countable items, looks up TYPICAL_WEIGHT_G and outputs
//     in grams; falls through to 100g default with the bare name otherwise
//   - Returns null for lines that have no number at all (headers, "salt to
//     taste", "a handful of nuts", empty strings)
//   - Pluralised foods are singularised by stripping a trailing 's' for the
//     name lookup (rough, but the food-matcher's fuzzy match handles the
//     rest). The output `name` keeps the singularised form.
...
// All known unit tokens, lowercased, derived from UNIT_TO_G keys plus a few
// aliases not in UNIT_TO_G that we accept on input. The parser normalises
// to one of: g, kg, ml, l, tsp, tbsp, cup.
...
function normaliseUnit(token) {
  ...
  if (t === "dash" || t === "dashes") return "pinch";  // dash maps to pinch (1g)
  return t;  // already canonical: g, kg, ml, l, tsp, tbsp, cup, oz, lb, pinch
}

// Singularise a count-noun-style ingredient name by stripping a trailing 's'.
// Crude but adequate — the fuzzy food matcher handles the residual variation.
// Strip preparation modifiers (minced, chopped, finely, etc.) from both ends
// of a token list. Repeated until no more strip happens, so combinations like
// "finely chopped" or "freshly grated" disappear cleanly.
function stripPrepModifiers(tokens) {
  let arr = tokens.slice();
  let changed = true;
  while (changed && arr.length > 0) {
    changed = false;
    if (PREP_MODIFIERS.has(arr[arr.length - 1].toLowerCase())) {
      arr = arr.slice(0, -1);
      changed = true;
    }
    if (arr.length > 0 && PREP_MODIFIERS.has(arr[0].toLowerCase())) {
      arr = arr.slice(1);
      changed = true;
    }
  }
...
function singulariseName(name) {
  ...
  // Don't strip 's' from words ending in 'ss' (e.g. "grass") or 'us' (cactus)
  if (n.endsWith("ss") || n.endsWith("us")) return n;
  // Common irregulars — extend if real recipes hit edge cases
  if (n.endsWith("leaves")) return n.slice(0, -6) + "leaf";       // bay leaves -> bay leaf
  if (n.endsWith("oes") && n.length > 4) return n.slice(0, -2);    // tomatoes -> tomato
  if (n.endsWith("ies") && n.length > 4) return n.slice(0, -3) + "y";  // berries -> berry
  if (n.endsWith("s")) return n.slice(0, -1);
  return n;
}

// Parse the leading quantity from a string. Returns { amount, rest } or null.
// Handles: 50, 1.5, 1,5, 1/2, ½, 2-3 (lower), 2 to 3 (lower), 1 1/2 (mixed).
function parseQuantity(s) {
  let str = s.trim();
  if (!str) return null;

  // Unicode fraction at the very start (e.g. "½ tsp salt")
  const firstChar = str[0];
  if (UNICODE_FRACTIONS[firstChar] !== undefined) {
    return { amount: UNICODE_FRACTIONS[firstChar], rest: str.slice(1).trim() };
  }

  // Mixed: "1 1/2 cups" or "1 ½ cups"
  const mixedAscii = str.match(/^(\d+)\s+(\d+)\/(\d+)\b/);
  if (mixedAscii) {
    const whole = parseInt(mixedAscii[1], 10);
    const num   = parseInt(mixedAscii[2], 10);
    const den   = parseInt(mixedAscii[3], 10);
    if (den !== 0) {
      return { amount: whole + num / den, rest: str.slice(mixedAscii[0].length).trim() };
    }
  }
  const mixedUnicode = str.match(/^(\d+)\s+([\u00BC-\u00BE\u2153-\u215E])/);
  if (mixedUnicode) {
    const whole = parseInt(mixedUnicode[1], 10);
    const frac  = UNICODE_FRACTIONS[mixedUnicode[2]];
    if (frac !== undefined) {
      return { amount: whole + frac, rest: str.slice(mixedUnicode[0].length).trim() };
    }
  }

  // Range: "2-3 onions" or "2 to 3 onions" — take lower
  const dashRange = str.match(/^(\d+(?:[.,]\d+)?)\s*[-\u2013\u2014]\s*\d+(?:[.,]\d+)?/);
  if (dashRange) {
    const low = parseFloat(dashRange[1].replace(",", "."));
    return { amount: low, rest: str.slice(dashRange[0].length).trim() };
  }
  const wordRange = str.match(/^(\d+(?:[.,]\d+)?)\s+to\s+\d+(?:[.,]\d+)?/i);
  if (wordRange) {
    const low = parseFloat(wordRange[1].replace(",", "."));
    return { amount: low, rest: str.slice(wordRange[0].length).trim() };
  }
  ...
  if (frac) {
    ...
    if (den !== 0) {
      return { amount: num / den, rest: str.slice(frac[0].length).trim() };
    }
  }

  // Decimal or integer: "1.5", "1,5", "50"
  const num = str.match(/^(\d+(?:[.,]\d+)?)/);
  if (num) {
    const n = parseFloat(num[1].replace(",", "."));
    return { amount: n, rest: str.slice(num[0].length).trim() };
  }

  return null;
}

// Parse a single ingredient line. Returns { name, amount, unit } or null.
function parseIngredientLine(rawLine) {
  if (!rawLine || typeof rawLine !== "string") return null;

  // 1. Normalise: trim, collapse whitespace, strip leading qualifier
  let line = rawLine.replace(/\s+/g, " ").trim();
  if (!line) return null;
  line = line.replace(LEADING_QUALIFIERS, "");

  // Tilde with no space: "~50g" -> "50g"
  line = line.replace(/^~\s*/, "");

  // Leading parenthesis pattern: "(7 oz) button mushrooms" or "(2 cups) stock"
  // Strip the parens — the contents become the leading quantity+unit.
  const leadingParen = line.match(/^\(([^)]*)\)\s+/);
  if (leadingParen) {
    line = leadingParen[1].trim() + " " + line.slice(leadingParen[0].length).trim();
  }

  // 2. Detect trailing parenthetical with a weight/volume — used as an
  // explicit total override. Examples:
  //   "7 oz mushrooms (200g)"     → use 200g
  //   "1 cup flour (120g)"        → use 120g
  //   "1 stick leek (60g)"        → use 60g
  // Only triggers when the parenthetical is exactly a number + a known mass
  // or volume unit. Anything else ("sifted", "optional") is treated as an
  // annotation and stripped.
  let parentheticalOverride = null;
  const parenMatch = line.match(/\s*\(([^)]*)\)\s*$/);
  if (parenMatch) {
    const inner = parenMatch[1].trim();
    // Try to parse the parenthetical as a quantity + unit
    const innerQ = parseQuantity(inner);
    if (innerQ && innerQ.rest) {
      const innerUnitTok = innerQ.rest.split(/\s+/)[0].toLowerCase().replace(/[.,;:]+$/, "");
      // Only mass and volume are meaningful as overrides; tsp/tbsp/cup are
      // already non-ambiguous from the leading quantity, so don't override.
      const OVERRIDE_UNITS = new Set(["g","gram","grams","kg","ml","l","oz","ounce","ounces","lb","pound","pounds"]);
      if (OVERRIDE_UNITS.has(innerUnitTok)) {
        parentheticalOverride = {
          amount: innerQ.amount,
          unit:   normaliseUnit(innerUnitTok),
        };
      }
    }
    // Strip the parenthetical regardless — it's either an override (consumed)
    // or an annotation (discarded)
    line = line.slice(0, parenMatch.index).trim();
  }

  // Strip after-comma annotations only when the comma is preceded by a
  // letter and followed by whitespace — that distinguishes "flour, sifted"
  // (annotation) from "1,5" (European decimal).
  const annotComma = line.match(/^(.+?[A-Za-z]),\s/);
  if (annotComma) {
    line = annotComma[1].trim();
  }
  if (!line) return null;

  // 3. Pull off the quantity. If there's no number, check for a quantity-less
  // unit prefix like "pinch of salt" → treat as "1 pinch of salt".
  let amount, rest;
  let preconsumedUnit = "";
  ...
  if (q) {
    ...
    // Look for a quantity-less unit at the start
    const firstTok = line.split(/\s+/)[0].toLowerCase().replace(/[.,;:]+$/, "");
    if (KNOWN_UNITS.has(firstTok)) {
      amount         = 1;
      preconsumedUnit = normaliseUnit(firstTok);
      // Rest = everything after the first token
  ...
  // 4. Tokenise the rest. The first token may be a unit; if so, consume it.
  const tokens = rest.split(/\s+/);
  if (tokens.length === 0) return null;

  let unit = preconsumedUnit;
  let nameTokens = tokens.slice();

  if (!unit) {
    const firstLower    = tokens[0].toLowerCase();
    const firstStripped = firstLower.replace(/[.,;:]+$/, "");

    if (KNOWN_UNITS.has(firstStripped)) {
      unit = normaliseUnit(firstStripped);
      nameTokens = tokens.slice(1);
    }
  }

  // Strip "of" if it follows the unit (in either path)
  if (unit && nameTokens.length > 0 && nameTokens[0].toLowerCase() === "of") {
    nameTokens = nameTokens.slice(1);
  }

  // 5. If no unit was found, this is a unit-less countable item.
  if (!unit) {
    ...
    // Strip preparation modifiers ("minced", "finely", etc.) BEFORE count-noun
    // detection so that "garlic cloves crushed" → "garlic cloves" → cloves
    // gets detected as a trailing count noun.
    nameTokens = stripPrepModifiers(nameTokens);
    if (nameTokens.length === 0) return null;

    // Detect count-noun construction in either order:
    //   trailing: "3 garlic cloves"  → noun=clove, name=garlic
    //   leading:  "3 cloves garlic"  → noun=clove, name=garlic
    //   leading + 'of': "3 cloves of garlic" → noun=clove, name=garlic
    let countNoun = "";
    if (nameTokens.length >= 2 && COUNT_NOUNS.has(nameTokens[nameTokens.length - 1].toLowerCase())) {
      ...
      nameTokens = nameTokens.slice(1);
      // Strip "of" if it follows: "3 cloves of garlic" → "garlic"
      if (nameTokens.length > 0 && nameTokens[0].toLowerCase() === "of") {
        nameTokens = nameTokens.slice(1);
      }
    }
    ...
    const singular   = singulariseName(rawName);
    if (!singular) return null;

    let weight;
    let lookupKey;
    if (countNoun) {
      lookupKey = singulariseName(countNoun);
      const joined = `${singular} ${lookupKey}`;
      if (TYPICAL_WEIGHT_G[joined] !== undefined) weight = TYPICAL_WEIGHT_G[joined];
      else if (TYPICAL_WEIGHT_G[singular] !== undefined) weight = TYPICAL_WEIGHT_G[singular];
    } else if (size) {
      const joined = `${singular} ${size}`;
      if (TYPICAL_WEIGHT_G[joined] !== undefined) weight = TYPICAL_WEIGHT_G[joined];
      else if (TYPICAL_WEIGHT_G[singular] !== undefined) weight = TYPICAL_WEIGHT_G[singular];
    } else if (TYPICAL_WEIGHT_G[singular] !== undefined) {
      weight = TYPICAL_WEIGHT_G[singular];
    }

    if (weight === undefined) weight = 100;

    // Parenthetical override (e.g. "7 oz mushrooms (200g)" → 200g) takes
    // precedence over the count × typical-weight calculation.
    if (parentheticalOverride) {
      return {
        name:   singular,
  ...
  // 6. Have a unit. Output the name as-is (lowercased, trimmed).
  // Strip preparation modifiers from both ends first.
  nameTokens = stripPrepModifiers(nameTokens);
  const name = nameTokens.join(" ").toLowerCase().trim();
  if (!name) return null;

  // Parenthetical override applies here too — "1 cup flour (120g)" → 120g
  if (parentheticalOverride) {
    return { name, amount: parentheticalOverride.amount, unit: parentheticalOverride.unit };
  }
  return { name, amount, unit };
}

// Drop-in replacement for parseIngredientsWithClaude. Async to preserve
// signature compatibility with the call sites.
async function parseIngredientsLocal(ingredientLines) {
  ...
  for (const line of ingredientLines) {
    ...
    if (parsed) {
      ...
      // Preserve the original line in the output so the review screen has
      // something to display and the user can manually pick a match.
      out.push({ name: (line || "").trim().toLowerCase(), amount: 0, unit: "" });
    }
  }
  return out;
}

// Dispatcher — call sites use this. Switch behaviour via USE_REGEX_PARSER.
async function parseIngredients(ingredientLines) {
  if (USE_REGEX_PARSER) return parseIngredientsLocal(ingredientLines);
  return parseIngredientsWithClaude(ingredientLines);
}
...
const handleTestConnection = async () => {
  ...
  try {
    ...
    setNotionSyncMsg({ type:"info", text:`✓ Connected to Worker (v${data.version || "?"})` });
    setTimeout(() => setNotionSyncMsg(null), 4000);
  } catch (err) {
    const msg = err.message || "unknown error";
    let friendly;
    if (msg.includes("worker_403"))      friendly = "Worker rejected the request. Check that this Pages origin is in the Worker's ALLOWED_ORIGINS env var.";
    else if (msg.startsWith("network:")) friendly = "Could not reach the Worker. Are you online? Is the URL correct? Note: the Claude artifact runtime blocks fetches to *.workers.dev — this only works from the test harness or a deployed PWA.";
    else                                  friendly = `Connection failed: ${msg}`;
...
const handleWorkerSync = async () => {
  ...
  try {
    ...
    for (let i = 0; i < usable.length; i++) {
      const r = usable[i];
      try {
        const ingredients = await parseIngredients(r.ingredientLines);
        parsed.push({ ...r, ingredients });
      } catch (e) {
        // Parser failure for one recipe shouldn't kill the whole sync;
...
// Phase 5c parser-only test path. Bypasses parseRecipesFromPasteText (the
// Claude-API recipe-extraction step) and feeds raw ingredient lines directly
// to parseIngredients. Wraps the result in a synthetic single-recipe payload
// so the existing review screen can display it. Used for testing the regex
// parser in isolation inside the artifact runtime.
const handleParserTest = async () => {
  if (!parserTestText.trim()) return;
  setSyncInProgress(true);
  setNotionSyncMsg({ type:"info", text:"Running regex parser…" });
  try {
    const lines = parserTestText
      .split(/\r?\n/)
      .map(l => l.trim())
      .filter(Boolean);
    if (lines.length === 0) {
      setNotionSyncMsg({ type:"error", text:"No ingredient lines to parse." });
      setSyncInProgress(false);
      setTimeout(() => setNotionSyncMsg(null), 4000);
      return;
    }
    const ingredients = await parseIngredients(lines);
    const synthetic = [{
      title:           "Parser Test",
      servings:        1,
    ...
    buildReviewData(synthetic);
  } catch (err) {
    setNotionSyncMsg({ type:"error", text:`Parser failed: ${err.message || "unknown"}` });
    setSyncInProgress(false);
    setTimeout(() => setNotionSyncMsg(null), 5000);
  }
};
...
fontSize:13, color: notionSyncMsg.type==="error" ? "#fca5a5" : "#93c5fd" }}>
{notionSyncMsg.text}
</div>
)}
...
{/* Parser-only test (Phase 5c) — bypasses recipe extraction, runs only
    the regex ingredient parser. For testing inside the artifact runtime. */}
<details style={{ marginTop:6 }}>
<summary style={{ fontSize:12, color:"#64748b", cursor:"pointer", padding:"8px 0", listStyle:"none", outline:"none", userSelect:"none" }}>
▸ Test regex parser (dev)
</summary>
<div style={{ marginTop:8 }}>
<div style={{ fontSize:11, color:"#64748b", marginBottom:8, lineHeight:1.5 }}>
Paste raw ingredient lines (one per line). Skips recipe extraction; runs only the regex parser. Output appears in the review screen as a synthetic recipe called "Parser Test".
</div>
<textarea
style={{ width:"100%", background:"#1e293b", border:"1px solid #334155", borderRadius:10,
padding:"12px 14px", color:"#e2e8f0", fontSize:13, outline:"none",
...
fontSize:13, fontWeight:600, cursor: parserTestText.trim()&&!syncInProgress ? "pointer" : "default" }}
disabled={!parserTestText.trim()||syncInProgress}
onClick={handleParserTest}>
{syncInProgress ? "Working…" : "Run parser"}
</button>
</div>
</details>
...
/* ──────────────────────────────────────────────────────────────────────────
 * Phase 5c — regex parser test cases (revised after Phase 5b validation)
 * ──────────────────────────────────────────────────────────────────────────
 * 50/50 passed when last validated. To re-run, extract parseIngredientLine
 * (and its helpers + lookup tables) into a standalone Node script.
 *
 * BASIC CASES
 *   "50 g of flour"              -> { name:"flour",       amount:50,    unit:"g"     }
 *   "1 tsp of cornstarch"        -> { name:"cornstarch",  amount:1,     unit:"tsp"   }
 *   "90 g brown sugar"           -> { name:"brown sugar", amount:90,    unit:"g"     }
 *   "100 ml oat milk"            -> { name:"oat milk",    amount:100,   unit:"ml"    }
 *   "1/2 tsp salt"               -> { name:"salt",        amount:0.5,   unit:"tsp"   }
 *   "1 cup flour (sifted)"       -> { name:"flour",       amount:1,     unit:"cup"   }
 *   "about 50g almonds"          -> { name:"almonds",     amount:50,    unit:"g"     }
 *   "1 kg potatoes"              -> { name:"potatoes",    amount:1,     unit:"kg"    }
 *   "1 l water"                  -> { name:"water",       amount:1,     unit:"l"     }
 *   "½ tbsp olive oil"           -> { name:"olive oil",   amount:0.5,   unit:"tbsp"  }
 *   "1.5 cups flour"             -> { name:"flour",       amount:1.5,   unit:"cup"   }
 *   "0.5 kg rice"                -> { name:"rice",        amount:0.5,   unit:"kg"    }
 *   "1,5 cups flour"             -> { name:"flour",       amount:1.5,   unit:"cup"   }
 *   "1,25 kg apples"             -> { name:"apples",      amount:1.25,  unit:"kg"    }
 *   "1 1/2 cups water"           -> { name:"water",       amount:1.5,   unit:"cup"   }
 *   "100g flour, sifted"         -> { name:"flour",       amount:100,   unit:"g"     }
 *
 * NULL CASES (parser returns null, review screen flags as unparseable)
 *   "salt to taste"              -> null
 *   "For the topping:"           -> null
 *   "flour, sifted"              -> null  (annotation, no quantity)
 *   "a handful of nuts"          -> null
 *   ""                           -> null
 *   "  "                         -> null
 *
 * COUNT-NOUN CASES (lookup-based weight derivation)
 *   "2 medium onions"            -> { name:"onion",       amount:220,   unit:"g"     }
 *   "3 garlic cloves"            -> { name:"garlic",      amount:9,     unit:"g"     }
 *   "1 medium lemon"             -> { name:"lemon",       amount:65,    unit:"g"     }
 *   "2-3 onions"                 -> { name:"onion",       amount:220,   unit:"g"     }
 *   "2 stalks celery"            -> { name:"celery",      amount:200,   unit:"g"     }  (default 100g × 2)
 *   "2 bay leaves"               -> { name:"bay leaf",    amount:1,     unit:"g"     }  (×0.5g)
 *   "1 sprig thyme"              -> { name:"thyme",       amount:1,     unit:"g"     }
 *
 * BUG FIXES from Phase 5b validation against real Notion data:
 *
 * Leading count noun:
 *   "3 cloves garlic"            -> { name:"garlic",      amount:9,     unit:"g"     }
 *   "3 cloves of garlic"         -> { name:"garlic",      amount:9,     unit:"g"     }
 *   "3 cloves garlic minced"     -> { name:"garlic",      amount:9,     unit:"g"     }
 *
 * Imperial units (oz, lb):
 *   "7 oz mushrooms"             -> { name:"mushrooms",   amount:7,     unit:"oz"    }
 *   "1 lb potatoes"              -> { name:"potatoes",    amount:1,     unit:"lb"    }
 *   "7 ounces flour"             -> { name:"flour",       amount:7,     unit:"oz"    }
 *
 * Trailing parenthetical with weight (overrides leading quantity):
 *   "7 oz mushrooms (200g)"      -> { name:"mushrooms",   amount:200,   unit:"g"     }
 *   "1 cup flour (120g)"         -> { name:"flour",       amount:120,   unit:"g"     }
 *   "1 stick leek (60g)"         -> { name:"leek",        amount:60,    unit:"g"     }
 *   "3 garlic cloves (9g)"       -> { name:"garlic",      amount:9,     unit:"g"     }
 *
 * Leading parenthesis pattern:
 *   "(7 oz) button mushrooms"    -> { name:"button mushrooms", amount:7, unit:"oz"   }
 *   "(2 cups) vegetable stock"   -> { name:"vegetable stock",  amount:2, unit:"cup"  }
 *
 * Prep-modifier stripping:
 *   "100g chopped onion"         -> { name:"onion",       amount:100,   unit:"g"     }
 *   "3 garlic cloves crushed"    -> { name:"garlic",      amount:9,     unit:"g"     }
 *   "1 tbsp olive oil, freshly cold-pressed" -> { name:"olive oil", amount:1, unit:"tbsp" }
 *
 * Quantity-less unit prefix:
 *   "pinch of salt"              -> { name:"salt",        amount:1,     unit:"pinch" }
 *   "dash of vanilla"            -> { name:"vanilla",     amount:1,     unit:"pinch" }
 *
 * RELATED FUZZY-MATCHER FIX (not parser, but bundled with this iteration):
 * fuzzyMatchFood now rejects matches where the ingredient name has a trailing
 * food-identity modifier (vinegar, juice, oil, paste, sauce, powder, extract,
 * syrup, butter, cream, milk, flour, stock, broth, wine) that the food name
 * doesn't include. So:
 *   "red wine vinegar"  no longer matches food "Red Wine"  → no match
 *   "lemon juice"       no longer matches food "Lemon"     → no match
 *   "tomato sauce"      no longer matches food "Tomato"    → no match
 *   "olive oil"         still matches food "Olive Oil"     → match
 *   "coconut milk"      still matches food "Coconut Milk"  → match
 */
