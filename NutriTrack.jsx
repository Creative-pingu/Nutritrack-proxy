        ...
        <div style={S.section}>
          ...
          {validationWarning && (
            <div style={{background:"#1c1000",border:"1px solid #92400e",borderRadius:12,padding:"12px 14px",marginBottom:10}}>
              <div style={{fontSize:13,color:"#fbbf24",fontWeight:600,marginBottom:6}}>⚠️ Corrupted storage key detected</div>
              <div style={{fontSize:12,color:"#92400e",marginBottom:10,lineHeight:1.5}}>Tap below to delete the bad key and reload. This clears only the corrupted key — all other data is untouched.</div>
              <button
                ...
                onClick={()=>{
                  // Remove only the keys that are in corruptedKeys (parse errors) or known bad shapes
                  // For safety, only remove nt-logs if it is actually unparseable right now
                  const keysToRemove = Object.values(STORAGE_KEYS).filter(k => {
                    try { const r = localStorage.getItem(k); if (r === null) return false; JSON.parse(r); return false; }
                    catch { return true; }
                  });
                  if (keysToRemove.length === 0) {
                    // Shape mismatch case — nt-logs is valid JSON but wrong shape; remove it
                ...
                Clear corrupted key and reload
              </button>
            </div>
          )}
          <details>
