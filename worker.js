...
export default {
  async fetch(request, env) {
    ...
    try {
      // CORS preflight — handled before auth so browsers don't choke on it
      ...
      if (request.method === "POST" && path === "/recipes/list") {
        const data = await listRecipes(env);
        return jsonResponse(data, 200, origin, env);
      }

      if (request.method === "POST" && path === "/recipes/page") {
        let body = {};
        try { body = await request.json(); } catch { /* body optional handled below */ }
      ...
      // Map Notion errors to 502; everything else is a 500
...
function isOriginAllowed(origin, env) {
  ...
  // Allow "*" as an emergency wildcard (useful while figuring out artifact-runtime origin)
...
function extractTitle(properties) {
  // Title is identified by type, not name — survives renaming
...
async function listRecipes(env) {
  ...
  const MAX_PAGES = 20;  // safety cap: 20 * 100 = 2000 recipes
...
async function fetchRecipePage(pageId, env) {
  ...
  // Rows of that table (table rows are themselves children blocks)
  ...
  // Skip header row, take first cell of each remaining row, drop blanks
