...
//
// Security model: Origin allowlist ONLY.
//
// The Origin header is set by the browser and cannot be spoofed by a real
// browser, but it CAN be spoofed by any non-browser HTTP client (curl,
// scripts, etc.). This is therefore not real security against a determined
// attacker — it only stops casual abuse from real browsers on other sites.
//
// This is an intentional choice. The recipe data is not sensitive, the
// Notion integration is read-only, and the JSX (which previously held a
// shared secret) is hosted on a public GitHub Pages repo where the secret
// could be read directly. A shared secret in client code provides no real
// security in that arrangement.
//
// Reversibility: if the use case changes and recipe data needs to be
// genuinely protected, the upgrade path is:
//   1. Move Pages to a private repo (GitHub Pro)
//   2. Add an AUTH_SECRET env var here, re-add the header check
//   3. Add WORKER_AUTH_SECRET to the JSX, re-add the header on outgoing requests
